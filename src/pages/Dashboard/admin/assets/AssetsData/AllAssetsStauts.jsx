...
const AllAssetsStauts = () => {
    ...
    const AllStauts = [{key:"all" , name:"الكل"} , {key:"pending" , name:"قيد المراجعة"} , {key:"reject" , name:"تم الرفض"} ,{key:"fulfilled" , name:"تمت المراجعه"}]
           ...
           
            {
              ...
              className={`px-3 py-1 rounded-md text-white whitespace-nowrap min-w-[120px] text-center
                ${row?.status === "pending" ? "bg-yellow-500" :
                  row?.status === "reject" ? "bg-red-500" :
                  "bg-green-500"}`}
            >
              {row?.status === "pending" ? "قيد المراجعة" :
                row?.status === "reject" ? "تم الرفض" :
                "تمت المراجعة"}
            </span>
          },
