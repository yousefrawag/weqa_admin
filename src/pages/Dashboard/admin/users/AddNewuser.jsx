...
const AddNewuser = () => {
  ...
  const usersType = [
    { name: 'مسؤل', key: 'manager' },
    { name: 'تابع لمنشأه', key: 'user' },
  ];
  const [type, setType] = useState('admin');
  const [value, setvalue] = useState('');

  const Levels = [
    { name: 'هيكل رئيسى', key: 'maincategories' },
    { name: 'هيكل فرعى', key: 'categories' },
    { name: 'هيكل فرعى ثالث', key: 'subcategories' },
    { name: 'هيكل فرعى رابع', key: 'nestsubcategories' },
  ];
  ...
  const handelSubmit = (e) => {
    ...
    try {
      ...
      // Only add permissions if the role is not "owner"
      if (data.role !== 'owner') {
        if (
          data.role === 'manager' ||
          data.role === 'user' ||
      ...
      const validateUser = () => {
        if (type === 'manager') {
          ...
          if (data.role === 'employee' && !data.permissions)
            return 'يجب اختيار صلاحية المستخدم';
      ...
      addIteam(data, {
        ...
        onError: (error) => {
          return toast.error(
            error?.response?.data?.errors[0]?.msg || 'هناك خطأ ما',
          );
        },
      });
    } catch (error) {
  ...
  return (
    <div className="w-full">
      ...
      <form onSubmit={handelSubmit} className="w-full h-full shadow-md p-5	">
        ...
        <div>
          <span className="w-full text-lg font-medium text-gray-700 dark:text-white mb-5">
            نوع المستخدم
          </span>
            ...
            <>
              <div className="mb-6 flex flex-col  gap-5">
                ...
                >
                  ...
                  <option value="manager">مدير المنشأه</option>
