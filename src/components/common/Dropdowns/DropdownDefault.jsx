...
const DropdownDefault = ({ row, isOwner, iscanPut, iscanDelete, UpdateStuts, loaddingDelete  , status}) => {
  ...
  return (
    <div className="relative flex">
      ...
      >
        ...
        {
          status?.filter((item)=> item.key !== "all").map((item) => {
            return   <button
            key={item.key}
            value={item.key}
            className={`flex items-center p-2 hover:bg-main text-white w-full text-left 
            
            `}
            onClick={() => UpdateStuts(row._id , item.key)}
          >
           {item?.name}
          </button>
          })
