...
const EstbilshAbout = ({OverviewData}) => {
  return (
    <div className="mt-10 grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-3 2xl:gap-7.5 w-full">
            ...
            <CardEstbilsh  title="إجمالى الإصول" to={`/establishments-assets/${OverviewData?._id}`} total={30} />
            <CardEstbilsh  title="إجمالى المستخدمين" to="/establishments/23/users" total={20} />
            <CardEstbilsh  title="إجمالى المواقع" to={`/establishments-location/${OverviewData?._id}`} total={40} />
